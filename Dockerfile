...
# then log output of file /code/bin/mybinary
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/mybinary \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/mybinary | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/mybinary /usr/bin/mybinary
